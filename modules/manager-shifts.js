...
window.showCreateShiftModal = async function () {
  try {
      ...
      try {
        ...
        if (!location_id) {
          ...
          if (existing?.id) {
            ...
            if (typeof window.showMessage === 'function') window.showMessage(`✅ Location created: ${created.name}`, 'success');
          }
        }
        ...
        if (typeof window.showMessage === 'function') window.showMessage('✅ Shift created!', 'success');
        window.closeModal();
        ...
        console.error(err);
        if (typeof window.showMessage === 'function') window.showMessage('❌ ' + (err.message || 'Error creating shift'), 'error');
        else alert(err.message || 'Error creating shift');
      } finally {
    ...
    console.error(err);
    if (typeof window.showMessage === 'function') window.showMessage('❌ ' + (err.message || 'Could not open create shift'), 'error');
    else alert(err.message || 'Could not open create shift');
  }
};
